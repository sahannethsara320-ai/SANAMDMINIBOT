...
const BOT_INFO = {
    CHANNEL_LINK: 'https://whatsapp.com/channel/0029Vb7uex51iUxjEPCWJb3u',
    LOGO_URL: LOGO_URLS[Math.floor(Math.random() * LOGO_URLS.length)],
...
*© Powered by SANA MD* 🇱🇰`,
    ...
    menu: (prefix) => {
        ...
        // 🔍 Get plugins from your global commands collection
        const plugins = global.plugins || global.commands || [];
        ...
        for (const plugin of pluginList) {
            ...
            const command = plugin.command || plugin.name;
            if (!command) continue;
...
async function arslanmd(conn) {
    ...
    try {
        ...
        // Send startup message to the connected user (bot owner/session user)
