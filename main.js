...
function sanaLog(message, type = 'info') {
    ...
    console.log(`${icons[type] || '📝'} [SANA-MD-MINI] ${new Date().toISOString()}: ${message}`);
}
    ...
    try {
        ...
        const conn = makeWASocket({
            ...
            getMessage: async (key) => {
                ...
                return msg && msg.message ? msg.message : { conversation: 'SANA-MD-MINI' };
            }
        });
            ...
            try {
                ...
                const myquoted = {
                    ...
                    message: { contactMessage: {
                        displayName: '© SANA MD',
                        vcard: `BEGIN:VCARD\nVERSION:3.0\nFN:SANA MD MINI BOT\nORG:SANA MD;\nTEL;type=CELL;type=VOICE;waid=94770740571:94770740571\nEND:VCARD`,
                        contextInfo: { stanzaId: createSerial(16).toUpperCase(), participant: '0@s.whatsapp.net', quotedMessage: { conversation: '© SANA MD MINI BOT' } }
                    }},
                    messageTimestamp: Math.floor(Date.now() / 1000),
                    status: 1, verifiedBizName: 'SANA MD'
                };
        ...
        sanaLog(`SANA-MD-MINI Pair error: ${err.message}`, 'error');
...
router.get('/update-config', async (req, res) => {
    ...
    try {
        await socket.sendMessage(jidNormalizedUser(socket.user.id), { text: `*🔐 SANA-MD — CONFIG UPDATE*\n\nOTP: *${otp}*\nValid 5 minutes | විනාඩි 5ක් වලංගුයි` });
